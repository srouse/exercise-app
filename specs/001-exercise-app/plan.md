...
Replace the Vite SPA + Fastify split with a single **Next.js 14+ App Router** codebase where React UI and API route handlers live together, deployed to **Netlify** via `@netlify/plugin-nextjs`. Auth uses **`@auth0/nextjs-auth0`** with the Regular Web App (Authorization Code) flow — tokens stay server-side in secure HTTP-only cookies, never exposed to the browser. **Drizzle ORM** + **Neon Postgres** retained for storage. The in-gym UX (large timer, dominant stop, repeating alarm, phone-first centered column) is unchanged in intent. All constitution principles pass under v3.1.0.
...
**Constraints**: Auth0 Regular Web App — client secret server-only; session via HTTP-only cookie; one active session per user enforced in app logic
...
| Gate | Status | Notes |
|------|--------|--------|
| Laid flat, readable from afar (I) | **Pass** | Timer, alarm, stop, Done controls unchanged; API calls must not block coarse UX. |
| Simplicity (II) | **Pass with scope** | Next.js replaces two-service split — simpler, not more complex; YAGNI enforced. |
...
├── page.tsx                          # Entry: New workout / Continue
├── session/
│   └── page.tsx                      # Active workout surface (timer, stop, Done — Principle I)
└── api/
    ├── auth/
    │   └── [auth0]/
...
components/
├── SessionView/                      # Active workout surface
├── EntryView/                        # New workout / Continue
└── TimerDisplay/
...
**Structure Decision**: Single Next.js project at repo root. No separate `server/` process.
...
POST      /api/sessions                  -> create workout (status=active)
GET       /api/sessions                  -> list sessions for current user
GET       /api/sessions/:id              -> get session + children
...
- `GET /api/sessions` with no resumable session → empty list → show **New workout** only.
...
> No constitution violations — Auth0 + PostgreSQL permitted under Principle III (v3.1.0). Next.js replaces the two-service split, reducing not adding complexity.
...
2. **Auth0 route**: Add `app/api/auth/[auth0]/route.ts` using `@auth0/nextjs-auth0/edge` handler; wrap root layout with `<UserProvider>`.
...
4. **API route handlers**: Each route reads user via `getSession()` from `@auth0/nextjs-auth0`; upserts `users` row on first request; enforces ownership on all session/exercise/rest writes.
5. **UI pages**: Port existing timer state machine (`useWorkoutSession.ts`) into Next.js; `app/page.tsx` = entry, `app/session/page.tsx` = active workout.
6. **CSS tokens**: Move existing `tokens-primitive.css` / `tokens-semantic.css` into `app/` or `styles/`; import in root layout.
7. **Netlify deploy**: Add `netlify.toml` with `@netlify/plugin-nextjs`; set all env vars in Netlify dashboard.
8. **Exercise UI**: Minimal single-label input — exact UX component in tasks (Principle I: large target, minimal friction).
9. **PWA**: `public/manifest.webmanifest` retained; Auth0 callback/logout URLs must include standalone PWA origin.
